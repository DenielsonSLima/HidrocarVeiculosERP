...
import OpcionaisForm from './components/OpcionaisForm';
import OpcionaisList from './components/OpcionaisList';
import ConfirmModal from '../../../components/ConfirmModal';

const OpcionaisPage: React.FC = () => {
  const navigate = useNavigate();
  const [items, setItems] = useState<IOpcional[]>([]);
  const [loading, setLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<IOpcional | null>(null);
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      const data = await OpcionaisService.getAll();
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadData();
    const sub = OpcionaisService.subscribe(() => loadData(true));
  ...
  const handleEdit = (item: IOpcional) => {
    setEditingItem(item);
    setIsFormOpen(true);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await OpcionaisService.remove(deleteId);
      showToast('success', 'Opcional excluído com sucesso.');
      loadData(true);
      setDeleteId(null);
    } catch (err: any) {
  ...
  const handleSubmit = async (data: Partial<IOpcional>) => {
    setIsSaving(true);
    try {
      await OpcionaisService.save(data);
      setIsFormOpen(false);
      showToast('success', data.id ? 'Opcional atualizado!' : 'Novo opcional salvo!');
      loadData(true);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tighter">Opcionais</h1>
          <p className="text-slate-500 mt-1">Gerencie acessórios, itens de conforto e segurança.</p>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 min-h-[500px]">
        <div className="mb-8 relative max-w-md">
          ...
          <input 
            type="text" 
            placeholder="Pesquisar opcional (ex: Teto Solar)..."
            value={searchTerm}
