...
interface Props {
  items: ICor[];
  onEdit: (item: ICor) => void;
  onDelete: (id: string) => void;
}

const CoresList: React.FC<Props> = ({ items, onEdit, onDelete }) => {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {items.map(item => (
        <div key={item.id} className="group relative bg-slate-50 p-3 sm:p-4 rounded-[1.5rem] border border-slate-100 flex items-center justify-between hover:bg-white hover:border-indigo-200 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-3 min-w-0 pr-12 w-full">
            {/* Amostra de Cor */}
            <div 
              className="w-8 h-8 rounded-full border-2 border-white shadow-md shrink-0 ring-1 ring-slate-100"
              style={{ backgroundColor: item.rgb_hex }}
              title={item.rgb_hex}
            ></div>
