...
interface Props {
  initialData: ICombustivel | null;
  isSaving: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<ICombustivel>) => void;
}

const CombustivelForm: React.FC<Props> = ({ initialData, isSaving, onClose, onSubmit }) => {
        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">
              {initialData ? 'Editar Combustível' : 'Novo Combustível'}
            </h2>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Alimentação do Veículo</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
            ...
            <input 
              ...
              placeholder="Ex: Gasolina, Flex, Diesel S10, Elétrico..."
              required
            />
          </div>
...
export default CombustivelForm;
