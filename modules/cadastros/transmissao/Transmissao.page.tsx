...
import TransmissaoForm from './components/TransmissaoForm';
import TransmissaoList from './components/TransmissaoList';
import ConfirmModal from '../../../components/ConfirmModal';

const TransmissaoPage: React.FC = () => {
  const navigate = useNavigate();
  const [items, setItems] = useState<ITransmissao[]>([]);
  const [loading, setLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<ITransmissao | null>(null);
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      const data = await TransmissaoService.getAll();
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadData();
    const sub = TransmissaoService.subscribe(() => loadData(true));
  ...
  const handleEdit = (item: ITransmissao) => {
    setEditingItem(item);
    setIsFormOpen(true);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await TransmissaoService.remove(deleteId);
      showToast('success', 'Transmissão excluída com sucesso.');
      loadData(true);
      setDeleteId(null);
    } catch (err: any) {
  ...
  const handleSubmit = async (data: Partial<ITransmissao>) => {
    setIsSaving(true);
    try {
      await TransmissaoService.save(data);
      setIsFormOpen(false);
      showToast('success', data.id ? 'Transmissão atualizada!' : 'Nova transmissão salva!');
      loadData(true);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tighter">Transmissão</h1>
          <p className="text-slate-500 mt-1">Gerencie os tipos de câmbio dos veículos.</p>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 min-h-[500px]">
        <div className="mb-8 relative max-w-md">
          ...
          <input 
            type="text" 
            placeholder="Pesquisar (ex: Manual, CVT)..."
            value={searchTerm}
