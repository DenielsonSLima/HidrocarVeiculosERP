...
import { MotorizacaoService } from './motorizacao.service';
import { IMotorizacao } from './motorizacao.types';
import MotorizacaoForm from './components/MotorizacaoForm';
import MotorizacaoList from './components/MotorizacaoList';
import ConfirmModal from '../../../components/ConfirmModal';

const MotorizacaoPage: React.FC = () => {
  const navigate = useNavigate();
  const [items, setItems] = useState<IMotorizacao[]>([]);
  const [loading, setLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<IMotorizacao | null>(null);
  ...
  const handleEdit = (item: IMotorizacao) => {
    setEditingItem(item);
    setIsFormOpen(true);
  };
  ...
  const handleSubmit = async (data: Partial<IMotorizacao>) => {
    setIsSaving(true);
    try {
      await MotorizacaoService.save(data);
      setIsFormOpen(false);
      showToast('success', data.id ? 'Motorização atualizada!' : 'Nova motorização salva!');
      loadData(true);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tighter">Motorização</h1>
          <p className="text-slate-500 mt-1">Gerencie os tipos de motores e cilindradas.</p>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 min-h-[500px]">
        ...
        {loading ? (
          ...
          <div className="py-20 text-center text-slate-400 italic">Nenhum motor cadastrado.</div>
        ) : (
          <MotorizacaoList 
            items={filteredItems} 
            onEdit={handleEdit} 
...
export default MotorizacaoPage;
