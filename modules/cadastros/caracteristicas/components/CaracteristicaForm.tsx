...
interface Props {
  initialData: ICaracteristica | null;
  isSaving: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<ICaracteristica>) => void;
}

const CaracteristicaForm: React.FC<Props> = ({ initialData, isSaving, onClose, onSubmit }) => {
        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">
              {initialData ? 'Editar Característica' : 'Nova Característica'}
            </h2>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Selo de qualidade do veículo</p>
          </div>
