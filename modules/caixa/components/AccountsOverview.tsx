        ...
        {contas.map(c => (
          <div key={c.id} className="p-5 rounded-3xl border border-slate-100 bg-slate-50 hover:bg-white hover:border-indigo-200 hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-[8px] font-black px-2 py-0.5 rounded uppercase ${c.status === 'ATIVA' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-200 text-slate-500'}`}>
                {c.status || 'INATIVA'}
              </span>
            </div>
            ...
            <h4 className="text-sm font-bold text-slate-800 mb-2 truncate" title={c.descricao}>{c.descricao}</h4>
